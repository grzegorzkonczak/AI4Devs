...
## JSONL write (one JSON doc per line)

Standard format for storing chunks / records � each line is an independent, index-ready document:

```js
const toJsonl = (rows) => rows.map((r) => JSON.stringify(r)).join("\n")
await writeFile(`workspace/example-${name}.jsonl`, toJsonl(chunks), "utf-8")
```
